 ...
 * - 同步：默认连接官方 GunDB 中继（开箱即用），可在界面改为私有中继
 * ===================================================================== */
...
/* ---------- 中继地址（可配置） ---------- */
// 默认出厂值：官方 Render 中继 —— 便于【免费用户开箱即用】，打开即连、无需手动填。
// 付费 / VIP 私有部署：应在各自环境把本常量改为客户自己的专属中继（或由用户在界面「修改中继地址」改填），
// 从而实现数据隔离（不与其他部署共用官方中继）。
// 注意：GunDB 中继是 Node 服务，免费静态托管（GitHub Pages 等）跑不了，须放能跑 Node 的环境。
const RELAY_URL = 'https://chat4hub-relay.onrender.com/gun';   // 非空 = 出厂即连官方中继；打开网页时输入框预填此地址
...
// —— 中继连接保活与自动重连 ——
// 设计原则（吸取 b91e261 教训）：
//   1) 稳定优先：gun 实例一旦建好且中继地址未变，就【永远复用，绝不重建】。
//      浏览器里 Gun 中继的 hi/bye 事件极不可靠（实测本地 multicast 假 peer 的 bye 反而频繁触发），
//      绝不能让它驱动「是否重建」，否则会反复 gun=Gun() 重建 → 连接震荡 → 收不到他人消息。
//   2) 只在【保活 ping 连续失败】时才认为中继真死，才重建一次（绝不用 gun.off() 拆共享 hub）。
...
function startKeepAlive() {
  ...
  const tick = () => {
    ...
    fetch(base + '/healthz', { mode: 'no-cors', cache: 'no-store' })
      ...
      .catch(() => {
        _kaPingFail++;
        // 连续 2 次 ping 失败（约 20min）+ 近期（>20min）没收到过他人消息 → 中继真死了，重建一次
        if (_kaPingFail >= 2 && Date.now() - _lastRemoteTs > 20 * 60 * 1000) {
          _kaPingFail = 0;
          gun = null; connectGun();
        }
      });
  ...
  tick();
  _kaTimer = setInterval(tick, 10 * 60 * 1000);   // 每 10 分钟
}
...
function connectGun() {
  ...
  gun.get('web3chat').map().on((data) => {
    ...
    if (isRemote) { _lastRemoteTs = Date.now(); }
